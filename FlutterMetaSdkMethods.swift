...
class FlutterMetaSdkMethods {
    ...
    static func purchased(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
       ...
       let rawParameters = arguments["parameters"] as? [AppEvents.ParameterName: Any] ?? [AppEvents.ParameterName: Any]()
        let parameters: [String: Any] = Dictionary(uniqueKeysWithValues: rawParameters.map { (key, value) in (key.rawValue, value) })
